...
public class TwitterClient extends SocialMediaClientBase {
	 ...
	 */
	TwitterJsonResponse doDeserialize(String jsonString, String url)
			throws ServiceUnavailableException {
		// Sample Twitter JSON:
		...
		if (jsonString != null
				&& jsonString
						.contains("Unable to access URL counting services")) {
			// Sample error JSON:
			// {"errors":[{"code":48,"message":"Unable to access URL counting services"}]}
			throw new ServiceUnavailableException(
					"Unable to access URL counting services for URL " + url);
		}
